...
console.log(
  ...
  // ========================================================
// SWITCH BACK TO PARTY A FOR ACCEPT
// ========================================================

console.log(
    "\n=== Switching to Party A for Accept ==="
  );
  
  const switchPartyAForAccept =
    await sendAutomation(
      page,
      {
        type: "SUI_TEST_WALLET_AUTOMATION",
        action: "SET_ACTIVE_ACCOUNT",
        id: "switch-party-a-for-accept",
        address: E2E_ADDRESS,
      }
    );
  
  expect(
    switchPartyAForAccept.success
  ).toBe(true);
  ...
  await disconnectForAccept.click();
  ...
  await connectForAccept.click();
  
  const walletForAccept =
    page.getByText(
      "Sui Test Wallet",
      { exact: true }
    );
  ...
  // ========================================================
  // ACCEPT FINALIZATION - PARTY A
  // ========================================================
  
  console.log(
    "\n=== Party A Accept Finalization ==="
  );
  ...
  console.log(
    "✓ PASS: Accept Finalization button visible"
  );
  
  const acceptApprovalPromise =
    context.waitForEvent("page");
  
  await acceptButton.click();
  
  console.log(
    "✓ PASS: Accept Finalization clicked"
  );
  
  const acceptApprovalPage =
    await acceptApprovalPromise;
  
  await acceptApprovalPage.waitForLoadState();
  
  console.log(
    "✓ PASS: Accept approval opened"
  );
  
  const acceptApproveButton =
    acceptApprovalPage.locator(
      'button:has-text("APPROVE")'
    );
  
  await expect(
    acceptApproveButton.first()
  ).toBeVisible();
  
  await acceptApproveButton
    .first()
    .dispatchEvent("click")
    .catch((error) => {
      if (
  ...
  console.log(
    "✓ PASS: Accept approval triggered"
  );
  ...
  console.log(
    "✓ PASS: Escrow status is COMPLETED"
  );
  ...
  console.log(
    "✓ PASS: Finalization accepted successfully"
  );
...
console.log(
  "FULL ESCROW LIFECYCLE: PASS"
);

console.log(
  "Create → Deposit A → Deposit B → Suggest → Accept → Completed"
);
